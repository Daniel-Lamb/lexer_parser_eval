...
(* Match  tok from list against expected tok *)
let match_token (toks : token list) (tok : token) =
  ...
  | h :: t when h = tok ->
      (* If first matches, return remaining *)
      t
  | h :: _ ->
      (* If mismatch, raise err *)
      raise
...
and parse_LetExpr toks =
  ...
  (* Check for rec to mark as recursive *)
  let (toks, is_rec) =
    match lookahead toks with
...
(* Parse expressions like int or records etc *)
...
and parse_RecordBodyExpr toks =
  ...
  | Some (Tok_ID id) ->
      ...
      let toks = match_token toks Tok_Equal in
      let (toks, expr) = parse_expr toks in
      (* Check if there's another field w semicolom *)
...
(* Parse a mutop directive *)
let rec parse_mutop toks =
  match lookahead toks with
  | Some Tok_Def ->
      parse_DefMutop toks
  | Some Tok_DoubleSemi ->
      (* If we see ';;', it's a NoOp (no operation). *)
      let toks = match_token toks Tok_DoubleSemi in
      (toks, NoOp)
  | _ ->
      (* Otherwise parse an expression and expect ';;' after it. *)
      let (toks, expr) = parse_expr toks in
      let toks = match_token toks Tok_DoubleSemi in
      (toks, Expr expr)

(* Parse a definition mutop: def x = expr ;; *)
